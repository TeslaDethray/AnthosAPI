...
class PlantsController extends AppController {
  ...
  public function index() {
    $this->paginate = ['contain' => ['ScientificNames']];
  ...
  public function add() {
    ...
    if ($this->request->is('post')) {
      ...
      if ($this->Plants->save($plant)) {
        ...
        $this->Flash->error(
          __('The plant could not be saved. Please, try again.')
        );
      }
    }
    $scientificNames = $this->Plants->ScientificNames->find(
      'list',
      ['limit' => 200]
    );
  ...
  public function delete($id = null) {
    ...
    if ($this->Plants->delete($plant)) {
      ...
      $this->Flash->error(
        __('The plant could not be deleted. Please, try again.')
      );
    }
    $redirection = $this->redirect(['action' => 'index']);
    return $redirection;
